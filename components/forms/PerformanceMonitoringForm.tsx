...
import PerformanceMonitoringPrintView from './print/PerformanceMonitoringPrintView';
...
const renderField = (field: Topic | SubTopic, formData: Record<string, any>, handleChange: (key: string, value: any) => void, isReadOnly?: boolean) => {
    ...
    const commonProps = {
        name: key, value, required: field.required, disabled: isReadOnly,
        onChange: (e: React.ChangeEvent<any>) => handleChange(key, e.target.value),
    };
...
const PerformanceMonitoringForm: React.FC<Props> = ({ 
    onSubmit, 
    onSaveDraft,
    definition,
    initialData, 
  ...
  const isFormComplete = useMemo(() => {
    ...
    return isMainFormComplete && signature && signature.name && signature.date;
  }, [formData, definition]);
  ...
  const handleSignatureChange = (signer: 'assessorSignature', field: string, value: string) => {
    setFormData(prev => ({ ...prev, [signer]: { ...(prev[signer] || {}), [field]: value }}));
  };
  ...
  const formFooter = (
    <div className="flex justify-between items-center pt-4 print:hidden">
        <div>
            <span className="text-sm text-gray-500">รหัสแบบฟอร์ม ITD-2025-THP006</span>
        </div>
  ...
  return (
    <>
      ...
      <PerformanceMonitoringPrintView data={formData} definition={definition} />
    </>
  );
};

export default PerformanceMonitoringForm;