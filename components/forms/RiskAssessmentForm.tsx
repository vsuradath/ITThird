...
import RiskAssessmentPrintView from './print/RiskAssessmentPrintView';
...
const renderField = (field: Topic | SubTopic, formData: Record<string, any>, handleChange: (key: string, value: any) => void, isReadOnly?: boolean) => {
    ...
    switch (field.inputType) {
        ...
        default:
            // The riskId field should be read-only
            if (key === 'riskId') {
                return <TextInput {...commonProps} readOnly disabled />;
            }
            return <TextInput {...commonProps} />;
    }
};

const RiskAssessmentForm: React.FC<Props> = ({ 
    onSubmit, 
    onSaveDraft, 
    definition,
    initialData, 
  ...
  const [formData, setFormData] = useState<Record<string, any>>({
    riskId: `RISK-${Date.now().toString().slice(-6)}`,
    ...(initialData || {}),
  });
  ...
  const formFooter = (
    <div className="flex justify-between items-center pt-4 print:hidden">
        <div>
            <span className="text-sm text-gray-500">รหัสแบบฟอร์ม ITD-2025-THP004</span>
        </div>
  ...
  return (
    <>
      ...
      <RiskAssessmentPrintView data={formData} definition={definition} />
    </>
  );
};
