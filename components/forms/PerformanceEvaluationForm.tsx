...
const renderField = (field: Topic | SubTopic, formData: Record<string, any>, handleChange: (key: string, value: any) => void) => {
    ...
    switch (field.inputType) {
        ...
        case 'number':
             return <TextInput name={key} value={value} onChange={e => handleChange(key, e.target.value)} type="number" min="1" max="5" placeholder={field.details} required={field.required} />;
        default:
...
const PerformanceEvaluationForm: React.FC<Props> = ({ 
    onSubmit, 
    definition,
    initialData, 
    isReadOnly,
